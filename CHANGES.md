...
- [CHANGE] クライアントから取得した音声データの想定サイズが、最大サイズを超えていた場合にエラーにする
  - 最大サイズは 16777215 バイト
  - @Hexa
- [FIX] エラー時のステータスコードを正しく返すために、クライアントにレスポンスヘッダを返すタイミングを外部サービス接続成功後に修正する
  - @Hexa
...
- [UPDATE] go.mod の Go のバージョンを 1.26.5 にあげる
  - @Hexa
...
- [UPDATE] go.mod の Go のバージョンを 1.26.4 にあげる
  - @Hexa
...
- [UPDATE] go.mod の Go のバージョンを 1.26.3 にあげる
  - @Hexa
